...
(function () {
  ...
  // Array.Reduce step
  var greetings = names.reduce(function(greetingArrays, name) {
    var firstLetter = name.charAt(0).toLowerCase();
    if (firstLetter === 'j') {
      greetingArrays.bye.push(byeSpeaker.speakSimple(name));
    } else {
      greetingArrays.hello.push(helloSpeaker.speakSimple(name));
    }
    return greetingArrays;
  }, {hello: [], bye: []});
  greetings.hello.forEach(greeting => console.log(greeting));
  greetings.bye.forEach(greeting => console.log(greeting));
